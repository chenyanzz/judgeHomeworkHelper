...
func GetHomeworkList(t string) (m []map[string]string, err error) {
	...
	for {
		...
		//if strings.Index(string(body), "还没有布置过作业哦，快去选题组卷布置作业吧~") > 0 {
		//	break
		//}
		...
		if strings.Index(doc.Find(".stu_homework").Text(), "还没有布置过作业哦，快去选题组卷布置作业吧~") >= 0 ||
			strings.Index(doc.Find(".stu_homework").Text(),"当前无已结束的作业") >= 0{
			break
		}
...
func C_GetHomeworkFromUrl(url *C.char) *C.char{
